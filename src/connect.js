...
const validateTicket_Author = (auID, callback) => {
  ...
  collection.find({ authorID: auID }).toArray(async function (err, result) {
    if (err) {
      console.log(err);
    } else {
      return await callback(result);
...
module.exports = {
  ...
  newTicket,
  newTicketPanel,
  validateTicket_Guild,
  validateTicket_Channel,
  validateTicket_Author,
  validateTicketPanel_Guild,
  validateTicketPanel_Channel,
  validateTicketPanel_Author,
  ticketUpdateStatus_Close,
  ticketUpdateStatus_Reopen,
  ticketPanelUpdateStatus_Close,
  ticketPanelUpdateStatus_Reopen,
  updateTicketAdd,
  deleteTicket,
  deleteTicketPanel,
  newBlacklist,
